...
import {
  handleNumber,
  toFloatValueFixed,
} from 'vxe-pc-ui/packages/number-input/src/util'
...
import type {
  ...
  VxeModalConstructor,
  VxeModalMethods,
  VxeDatePickerDefines,
  VxeSelectConstructor,
  VxeSelectMethods,
  VxeTreeSelectConstructor,
  VxeTreeSelectMethods,
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const $xeTreeSelect = inject<
      (VxeTreeSelectConstructor & VxeTreeSelectMethods) | null
    >('$xeTreeSelect', null)
    ...
    const computeIsDatePickerType = computed(() => {
      ...
      return (
        isDateTimeType ||
        ['date', 'week', 'month', 'quarter', 'year'].indexOf(props.type) > -1
      )
    })
    ...
    const computeDateListValue = computed(() => {
      ...
      if (multiple && modelValue && isDatePickerType) {
        return XEUtils.toValueString(modelValue)
          .split(',')
          .map((item) => {
    ...
    const computeDateMultipleValue = computed(() => {
      ...
      const dateValueFormat = computeDateValueFormat.value
      return dateListValue.map((date) =>
        XEUtils.toDateString(date, dateValueFormat),
      )
    })
    ...
    const computeDateMultipleLabel = computed(() => {
      ...
      return dateListValue
        .map((date) => XEUtils.toDateString(date, dateLabelFormat))
        .join(', ')
    })
    ...
    const computeDateLabelFormat = computed(() => {
      ...
      if (isDatePickerType) {
        return (
          labelFormat || getI18n(`vxe.input.date.labelFormat.${props.type}`)
        )
      }
      return ''
    })
    ...
    const computeSelectDatePanelObj = computed(() => {
      ...
      if (isDatePickerType) {
        ...
        if (datePanelType === 'quarter' || datePanelType === 'month') {
          ...
          y = yearList.length
            ? `${yearList[0].year} - ${yearList[yearList.length - 1].year}`
            : ''
        } else {
          y = getI18n('vxe.datePicker.yearTitle', [year])
    ...
    const computeWeekHeaders = computed(() => {
      ...
      if (isDatePickerType) {
        ...
        return [{ label: getI18n('vxe.input.date.weeks.w') }].concat(
          dateHeaders,
        )
      }
      return []
    ...
    const computeMonthList = computed(() => {
      ...
      if (selectMonth && currentDate) {
        ...
        const selFullYear = XEUtils.getWhatYear(
          selectMonth,
          0,
          'first',
        ).getFullYear()
    ...
    const getNumberValue = (val: any) => {
      ...
      const restVal =
        type === 'float'
          ? toFloatValueFixed(val, digitsValue)
          : XEUtils.toValueString(val)
      if (
        exponential &&
        (val === restVal ||
          XEUtils.toValueString(val).toLowerCase() ===
    ...
    const focusEvent = (evnt: Event & { type: 'focus' }) => {
      ...
      if (isNumType) {
        reactData.inputValue = eqEmptyValue(inputValue)
          ? ''
          : `${XEUtils.toNumber(inputValue)}`
    ...
    const dateParseValue = (value?: VxeInputPropTypes.ModelValue) => {
      ...
      if (XEUtils.isValidDate(dValue)) {
        ...
        if (dateLabelFormat && type === 'week') {
          ...
          const weekDate = XEUtils.getWhatWeek(
            dValue,
            0,
            weekNum === 1
              ? (((6 + firstDayOfWeek) % 7) as VxeInputPropTypes.StartDay)
              : firstDayOfWeek,
            firstDayOfWeek,
          )
    ...
    const changeValue = () => {
      ...
      if (isDatePickerType) {
        ...
        reactData.inputValue = props.multiple
    ...
    const validMaxNum = (num: number | string) => {
      return (
        props.max === null ||
        XEUtils.toNumber(num) <= XEUtils.toNumber(props.max)
      )
    }
    ...
    const validMinNum = (num: number | string) => {
      return (
        props.min === null ||
        XEUtils.toNumber(num) >= XEUtils.toNumber(props.min)
      )
    }
    ...
    const dateCheckMonth = (date: Date) => {
      ...
      const weekStartDate = XEUtils.getWhatWeek(
        date,
        0,
        firstDayOfWeek,
        firstDayOfWeek,
      )
      const month = XEUtils.getWhatMonth(
        weekNum === 1 ? XEUtils.getWhatDay(weekStartDate, 6) : date,
        0,
        'first',
      )
    ...
    const dateChange = (date: Date) => {
      ...
      const firstDayOfWeek = computeFirstDayOfWeek.value
      if (props.type === 'week') {
        const sWeek = XEUtils.toNumber(
          props.selectDay,
        ) as VxeInputPropTypes.SelectDay
      ...
      if (multiple) {
        ...
        if (isDateTimeType) {
          ...
          const eqIndex = XEUtils.findIndexOf(dateListValue, (val) =>
            XEUtils.isDateSame(date, val, 'yyyyMMdd'),
          )
          if (eqIndex === -1) {
          ...
          handleChange(
            datetimeRest
              .map((date) => XEUtils.toDateString(date, dateValueFormat))
              .join(','),
            { type: 'update' },
          )
        } else {
          ...
          if (dateMultipleValue.some((val) => XEUtils.isEqual(val, inpVal))) {
            handleChange(
              dateMultipleValue
                .filter((val) => !XEUtils.isEqual(val, inpVal))
                .join(','),
              { type: 'update' },
            )
          } else {
    ...
    const afterCheckValue = () => {
      ...
      if (!inputReadonly) {
        if (isNumType) {
          ...
          if (inputValue) {
            let inpDateVal: VxeInputPropTypes.ModelValue = parseDate(
              inputValue,
              dateLabelFormat as string,
            )
    ...
    const keydownEvent = (evnt: KeyboardEvent & { type: 'keydown' }) => {
      ...
      if (isNumType) {
        ...
        if (
          ...
          !isAltKey &&
          !isMetaKey &&
          (globalEvents.hasKey(evnt, GLOBAL_EVENT_KEYS.SPACEBAR) ||
    ...
    const datePrevEvent = (evnt: Event) => {
      ...
      if (!isDisabledPrevDateBtn) {
        ...
        inputMethods.dispatchEvent(
          'date-prev',
          { viewType: datePanelType, viewDate, value, type },
          evnt,
        )
      }
    }
    ...
    const dateSelectEvent = (
      item:
        | VxeDatePickerDefines.DateYearItem
        | VxeDatePickerDefines.DateQuarterItem
        | VxeDatePickerDefines.DateMonthItem
        | VxeDatePickerDefines.DateDayItem,
    ) => {
    ...
    const dateMoveDay = (offsetDay: Date) => {
      if (!isDateDisabled({ date: offsetDay })) {
        ...
        if (
          !dayList.some((item) =>
            XEUtils.isDateSame(item.date, offsetDay, 'yyyyMMdd'),
          )
        ) {
    ...
    const dateMoveYear = (offsetYear: Date) => {
      if (!isDateDisabled({ date: offsetYear })) {
        ...
        if (
          !yearList.some((item) =>
            XEUtils.isDateSame(item.date, offsetYear, 'yyyy'),
          )
        ) {
    ...
    const dateMoveQuarter = (offsetQuarter: Date) => {
      if (!isDateDisabled({ date: offsetQuarter })) {
        const quarterList = computeQuarterList.value
        if (
          !quarterList.some((item) =>
            XEUtils.isDateSame(item.date, offsetQuarter, 'yyyyq'),
          )
        ) {
    ...
    const dateMoveMonth = (offsetMonth: Date) => {
      if (!isDateDisabled({ date: offsetMonth })) {
        ...
        if (
          !monthList.some((item) =>
            XEUtils.isDateSame(item.date, offsetMonth, 'yyyyMM'),
          )
        ) {
    ...
    const dateHourEvent = (
      evnt: MouseEvent,
      item: VxeDatePickerDefines.DateHourMinuteSecondItem,
    ) => {
    ...
    const dateOffsetEvent = (evnt: KeyboardEvent) => {
      ...
      if (isActivated) {
        evnt.preventDefault()
        const isLeftArrow = globalEvents.hasKey(
          evnt,
          GLOBAL_EVENT_KEYS.ARROW_LEFT,
        )
        const isUpArrow = globalEvents.hasKey(evnt, GLOBAL_EVENT_KEYS.ARROW_UP)
        const isRightArrow = globalEvents.hasKey(
          evnt,
        ...
        if (datePanelType === 'year') {
          let offsetYear = XEUtils.getWhatYear(
          ...
          let offsetQuarter = XEUtils.getWhatQuarter(
            datePanelValue || Date.now(),
            0,
            'first',
          )
          ...
          let offsetMonth = XEUtils.getWhatMonth(
            datePanelValue || Date.now(),
            0,
            'first',
          )
    ...
    const handleGlobalMousedownEvent = (evnt: Event) => {
      ...
      if (!isDisabled && isActivated) {
        reactData.isActivated =
          ...
          getEventTargetNode(evnt, panelWrapperElem).flag
        if (!reactData.isActivated) {
          // 如果是日期类型
          if (isDatePickerType) {
            if (visiblePanel) {
    ...
    const renderDateLabel = (
      ...
      if (festivalMethod) {
        ...
        const festivalItem = festivalRest
          ? XEUtils.isString(festivalRest)
            ? { label: festivalRest }
            : festivalRest
          : {}
        const extraItem = festivalItem.extra
                  ...
                  ? h(
                      'span',
                      {
                        class: [
                          ...
                          `overlap--${festivalLabels.length}`,
                        ],
                      },
                      festivalLabels.map((label) =>
                        h('span', label.substring(0, 3)),
                      ),
                    )
                  : h(
                      'span',
                      ...
                      'td',
                      {
                        class: {
                          ...
                          'is--selected': multiple
                            ? dateListValue.some((val) =>
                                XEUtils.isDateSame(val, item.date, matchFormat),
                              )
                            : XEUtils.isDateSame(
                                dateValue,
                                item.date,
          ...
          [
            ...
            h(
              ...
              weekDates.map((rows) => {
                ...
                const isHover = rows.some((item) =>
                  XEUtils.isDateSame(datePanelValue, item.date, matchFormat),
                )
                return h(
                  'tr',
                  rows.map((item) => {
                ...
                return h(
                  ...
                  rows.map((item) => {
                    return h(
                      ...
                      renderDateLabel(
                        item,
                        getI18n(`vxe.input.date.months.m${item.month}`),
                      ),
                    )
                  }),
                )
                ...
                return h(
                  ...
                  rows.map((item) => {
                    return h(
                      ...
                      renderDateLabel(
                        item,
                        getI18n(`vxe.input.date.quarters.q${item.quarter}`),
                      ),
                    )
                  }),
                )
                ...
                return h(
                  'li',
                  {
                    ...
                    class: {
                      'is--selected':
                        datetimePanelValue &&
                        datetimePanelValue.getHours() === item.value,
                    },
                    ...
                    return h(
                      'li',
                      {
                        ...
                        class: {
                          'is--selected':
                            datetimePanelValue &&
                            datetimePanelValue.getMinutes() === item.value,
                        },
                    ...
                    return h(
                      'li',
                      {
                        ...
                        class: {
                          'is--selected':
                            datetimePanelValue &&
                            datetimePanelValue.getSeconds() === item.value,
                        },
                        onClick: (evnt: MouseEvent) =>
        ...
        [
          h('i', {
            class: [
              ...
              showPwd
                ? getIcon().PASSWORD_INPUT_SHOW_PWD
                : getIcon().PASSWORD_INPUT_HIDE_PWD,
            ],
          }),
        ],
      )
    ...
    const renderSuffixIcon = () => {
      ...
      const isExtraBtn =
        isPawdType || isNumType || isDatePickerType || isSearchType
