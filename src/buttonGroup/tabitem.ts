...
export class TabItem extends Base {
  ...
  drag(event: MouseEvent) {
    ...
    if (index < currentIndex) {
      sArr = tabitems.slice(index, currentIndex + 1).filter((item) => {
        return item != this
      })
      sArr.forEach((item) => {
      ...
      let _arr = tabitems.filter((item) => {
        return !sArr.includes(item) && item != this
      })
      _arr.forEach((item) => {
        let root = item.getRef('root')
      ...
      sArr = tabitems.slice(currentIndex, index + 1).filter((item) => {
        return item != this
      })
      sArr.forEach((item) => {
        let root = item.getRef('root')
  ...
  getSlotItemStyle() {
    ...
    if (this.isDrag) {
      ...
      let leftNum =
        this.dragConfig.currentX -
        this.dragConfig.tabLeft -
        this.dragConfig.subLeft +
        this.tab.scrollLeft -
        20
      if (leftNum < 0) {
        leftNum = 0
  ...
  endDrag(event: MouseEvent) {
    ...
    if (index != currentIndex) {
      this.tab.tabitems.splice(
        currentIndex,
        0,
        this.tab.tabitems.splice(index, 1)[0],
      ) //
    }
    this.dragConfig.dragIndex = null
  }
  ...
  getLabel() {
    let label = this.config.label || '页签'
    return label //
  }
