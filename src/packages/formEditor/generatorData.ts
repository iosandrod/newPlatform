...
export default function (
  ...
  if (isCreateLabel) {
    node.label =
      node.label ||
      utils.transferData(lang, utils.transferLabelPath(node), locale)
    ...
    if (/^(select|cascader|region|date|time)$/.test(node.type)) {
      node.options.placeholder = utils.transferData(
        lang,
        'er.validateMsg.placeholder2',
        locale,
      )
    }
    if (/^(select|checkbox|radio)$/.test(node.type)) {
      node.options.otherPlaceholder = utils.transferData(
        lang,
        'er.validateMsg.placeholder3',
        locale,
      )
    }
