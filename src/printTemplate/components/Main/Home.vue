...
function initConfig() {
  ...
  if (props.preComponentData)
    store.commit('printTemplateModule/setComponentData', props.preComponentData)
  if (props.prePageConfig)
    store.commit('printTemplateModule/setPageConfig', props.prePageConfig)
  if (props.preDataSource)
    store.commit('printTemplateModule/setDataSource', props.preDataSource)
  if (props.preDataSet)
    store.commit('printTemplateModule/setDataSet', props.preDataSet)
}
...
function showUserGuide() {
  ...
  driver.addSteps([
    ...
    {
      attachTo: {
        element: document.querySelector('.roy-designer-header'),
        on: 'auto',
      },
      title: '打印模板设计器界面指引',
      text:
        '这是标题栏，左侧显示”打印模板设计器”字样和当前模板名称，右侧是开发者自定义插槽和夜间模式切换按钮。',
      buttons: [
        {
          action() {
            return this.next()
    ...
    {
      attachTo: {
        element: document.querySelector('.roy-designer-aside'),
        on: 'auto',
      },
      title: '打印模板设计器界面指引',
      text: '这是左侧面板，包含四个模块。',
    ...
    {
      attachTo: {
        element: document.querySelector('.roy-designer-main'),
        on: 'bottom-start',
      },
      title: '打印模板设计器界面指引',
