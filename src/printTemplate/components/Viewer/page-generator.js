...
class BasePageGenerator {
  ...
  generateNewElementWithStyledComponent(element, zIndex = 0) {
    ...
    // return newElement
    const { component, style } = element
    const Component = componentToStyled[component]
    const container = document.createElement('div')
    const vnode = createVNode(Component, style)
    render(vnode, container)
    const newEl = /** @type {HTMLElement} */ (container.firstElementChild)

    newEl.style.width = `${style.width}px`
    newEl.style.height = AUTO_PAGE_COMPONENT.includes(component)
      ? 'auto'
      : `${style.height}px`
    if (style.minHeight != null) {
      newEl.style.minHeight = `${style.minHeight}px`
    }
    newEl.style.position = 'absolute'
    newEl.style.left = `${style.left}px`
    newEl.style.top = `${style.top}px`
    newEl.style.transform = `rotate(${style.rotate}deg)`
