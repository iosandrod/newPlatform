...
import { GanttBarObject } from './hy-vue-gantt'
import {
  ChartRow,
  GanttBarEvent,
  GGanttChartEmits,
  GGanttChartEventsProps,
  GGanttChartProps,
} from './types'
...
export class Gantt extends Base {
  bars: GanttBarObject[] //
  config: any
  constructor(config) {
    super() //
  ...
  getGanttStart() {
    let start = this.config.chartStart || '2025-01-01'
    return start
  }
  getGanttEnd() {
    let end = this.config.chartEnd || '2025-12-31'
    return end
  }
  getPrecision() {
    let precision = this.config.precision || 'day'
    return precision
  }
  getLocale() {
    let locale = this.config.locale || 'zh-CN'
    return locale
  }
  getBarstartField() {
    let barStart = this.config.barStart || 'start'
    return barStart
  }
  getBarEndField() {
    let barEnd = this.config.barEnd || 'end'
    return barEnd
  }
  getMultiColumnLabel() {
    let multiColumnLabel = [
      {
        field: 'label',
      },
    ]
    return multiColumnLabel
  }
  initContext(bar) {}
  getBars() {
    let bars = this.config.bars || []
    return bars //
  }
  getColumns() {
    return [
  ...
  loadData() {}
  getEventAxisHeight() {
    let eventAxisHeight = this.config.eventAxisHeight || 25
    return eventAxisHeight
  }
  getHeaderHeight() {
    let showEventsAxis = this.config.showEventsAxis
    let height = 80
    if (showEventsAxis) {
      height += this.getEventAxisHeight()
    }
    return height
  }
  getGanttBasicConfig() {
    let _config: GGanttChartProps = {
      chartStart: this.getGanttStart(),
      chartEnd: this.getGanttEnd(),
      barStart: 'start',
      barEnd: 'end', //
      labelColumnTitle: 'label',
      labelColumnWidth: 100,
      labelResizable: true, //
      eventsAxisHeight: this.getEventAxisHeight(),
    }
    return _config
  }
  getGanttBaseEvents() {
    let _events: GGanttChartEventsProps = {
      //
    }
    return _events
  }
  getGanttRowProps(row) {
    console.log(row, 'testRow') //
    let bars = row.bars
    let children = row.children
    if (!children?.length && !bars?.length) {
      bars = [row] //
    }
    let _config: ChartRow = {
      id: row.id,
      //@ts-ignore
      key: row.id,
      label: row.label || row?.ganttBarConfig?.label, //
      bars: bars,
      children: children, //
