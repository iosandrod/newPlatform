...
import { defineVxeComponent } from '@/vxegrid/ui/src/comp'
...
import VxeSelectComponent from '@/select/selectCom'
// import VxeInputComponent from '../../input/src/input'
import VxeInputComponent from '@/input/inputCom' //
import type {
  VxePagerPropTypes,
  VxePagerConstructor,
  VxePagerEmits,
...
export default defineVxeComponent({
  ...
  setup(props, context) {
    ...
    const jumpPageEvent = (evnt: Event, currentPage: number) => {
      ...
      if (evnt && currentPage !== props.currentPage) {
        pagerMethods.dispatchEvent(
          'page-change',
          { type: 'current', pageSize: props.pageSize, currentPage },
          evnt,
        )
      }
    }
    ...
    const handlePrevPage = (evnt?: Event) => {
      ...
      if (currentPage > 1) {
        changeCurrentPage(
          Math.min(pageCount, Math.max(currentPage - 1, 1)),
          evnt,
        )
      }
    }
    ...
    const handleNextJump = (evnt?: Event) => {
      ...
      const numList = computeNumList.value
      changeCurrentPage(
        Math.min(props.currentPage + numList.length, pageCount),
        evnt,
      )
    }
    ...
    const pageSizeEvent: VxeSelectEvents.Change = (params) => {
      ...
      pagerMethods.dispatchEvent(
        'page-change',
        { type: 'size', pageSize, currentPage },
        params.$event,
      )
    }
      ...
      return h(
        ...
        [
          h('i', {
            class: [
              'vxe-pager--btn-icon',
              props.iconHomePage || getIcon().PAGER_HOME,
            ],
          }),
        ],
      )
      ...
      return h(
        ...
        [
          h('i', {
            class: [
              'vxe-pager--btn-icon',
              props.iconPrevPage || getIcon().PAGER_PREV_PAGE,
            ],
          }),
        ],
      )
          ...
          tagName
            ? h('i', {
                class: [
                  ...
                  props.iconJumpMore || getIcon().PAGER_JUMP_MORE,
                ],
              })
            : null,
          h('i', {
            class: [
              'vxe-pager--jump-icon',
              props.iconJumpPrev || getIcon().PAGER_JUMP_PREV,
            ],
          }),
        ],
      )
      ...
      return h(
        ...
        [
          ...
          h('i', {
            class: [
              'vxe-pager--jump-icon',
              props.iconJumpNext || getIcon().PAGER_JUMP_NEXT,
            ],
          }),
        ],
      )
      ...
      return h(
        ...
        [
          h('i', {
            class: [
              'vxe-pager--btn-icon',
              props.iconNextPage || getIcon().PAGER_NEXT_PAGE,
            ],
          }),
        ],
      )
      ...
      return h(
        ...
        [
          h('i', {
            class: [
              'vxe-pager--btn-icon',
              props.iconEndPage || getIcon().PAGER_END,
            ],
          }),
        ],
      )
    ...
    const renderNumber = (showJump?: boolean) => {
      ...
      const numberSlot = showJump
        ? slots.numberJump || slots['number-jump']
        : slots.number
      const nums = []
      const pageCount = computePageCount.value
      const numList = computeNumList.value
    ...
    const renderJump = (isFull?: boolean) => {
      ...
      const jumpSlot = isFull
        ? slots.fullJump || slots['full-jump']
        : slots.jump
      const pageCount = computePageCount.value
      if (jumpSlot) {
        return h(
